...
Normal workflow APIs and AI tools share `assertProjectAccess` and `assertWorkflowAccess`. Project managers and designers must be project members even when the request belongs to their company. Unavailable financial values are omitted or represented as `{ accessible: false }`, never as zero.
...
Tool DTOs exclude contacts, phones, tax numbers, addresses, attachment URLs and unrelated record fields. A server-side Numeric Grounding Guard removes or hides final-answer numbers that cannot be traced to authorized tool output.
...
- Per-minute requests, daily requests and daily Token usage are configurable and enforced before a run starts.
...
`ai_conversations`, `ai_messages`, `ai_runs`, `ai_tool_calls` and `ai_provider_checks` capture ownership, scope, model/mode, status, latency, full-run token counts, primary/fast/fallback counters, stable error codes and bounded tool summaries. They do not store API keys or authorization headers.
