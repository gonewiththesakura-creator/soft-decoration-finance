...
export default async function ResourcePage({ params, searchParams }: { params: Promise<{ resource: string }>; searchParams: Promise<{ new?: string }> }) {
  ...
  return <main className="content"><div className="page-heading"><div><div className="eyebrow">{definition.eyebrow}</div><h1>{definition.title}</h1><p className="page-description">{definition.description}</p></div></div><ResourceTable definition={definition} rows={data.rows as (Record<string, unknown> & { id: number })[]} canWrite={can(user, resource, "write")} role={user.role} autoCreate={query.new === "1"} /></main>;
}
