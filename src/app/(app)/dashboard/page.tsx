...
import { ExecutiveAmbientBackground } from "@/components/executive/executive-ambient-background";
...
export default async function DashboardPage() {
  ...
  if (data.isEmpty) return <ExecutiveAmbientBackground><main className="content operating-page executive-dashboard os-page-enter"><EmptyBusinessState canImport={can(user, "imports", "write")} canScan={user.role === "owner"} /></main></ExecutiveAmbientBackground>;
  ...
  const aiReady = aiBriefEnabled && getAIConfig().configured;
  ...
  const actions: ExecutiveAction[] = [
    ...data.pending.map((item) => ({ id: `pending-${item.kind}-${item.id}`, type: item.kind as "采购审批" | "付款审批", title: item.number, project: item.projectName, amountCents: item.amountCents, time: item.createdAt, timeLabel: "创建" as const, risk: item.amountCents >= 500_000_00 ? "high" as const : "medium" as const, reason: "申请已进入待审批队列", suggestion: "核对预算、票据与付款条件后人工决策", href: item.kind === "付款审批" ? `/payment-requests${user.role === "owner" || user.role === "finance" ? `?open=${item.id}` : ""}` : `/purchase-requests${user.role === "owner" ? `?open=${item.id}` : ""}` })),
    ...data.overdue.slice(0, 3).map((item) => ({ id: `overdue-${item.id}`, type: "逾期应收" as const, title: item.customerName, project: item.projectName, amountCents: item.amountCents, time: item.dueDate, timeLabel: "到期" as const, risk: item.overdueDays > 60 ? "high" as const : "medium" as const, reason: `超过计划收款日 ${item.overdueDays} 天`, suggestion: "确认回款承诺并安排今日催收", href: `/receivables?overdue=true&projectId=${item.projectId}` })),
  ...
  const projectRiskCount = analytics.projectRiskCount;
  const minimumBalance = analytics.cashflow.length ? Math.min(...analytics.cashflow.map((point) => point.balanceCents)) : null;
  const currentBalanceInsight = [
    ...(data.summary.balance !== null ? [`集团资金 · ${formatMoney(data.summary.balance, true)}`] : []),
    ...data.balances.slice(0, 3).map((item) => `${item.name} · ${formatMoney(item.balanceCents, true)}`),
    ...(analytics.executiveDetails.topAccount ? [`Top Account · ${analytics.executiveDetails.topAccount.companyName} / ${analytics.executiveDetails.topAccount.accountName} · ${formatMoney(analytics.executiveDetails.topAccount.balanceCents, true)}`] : []),
  ];
  const overdueInsight = analytics.executiveDetails.overdue ? [
    `逾期客户 · ${analytics.executiveDetails.overdue.customerCount} 家`,
    `最大逾期客户 · ${analytics.executiveDetails.overdue.maxCustomerName ?? "暂无"}`,
    `最大逾期金额 · ${formatMoney(analytics.executiveDetails.overdue.maxAmountCents, true)}`,
    `最长逾期 · ${analytics.executiveDetails.overdue.longestOverdueDays} 天`,
  ] : [];
  const approvalInsight = [
    ...(data.summary.purchaseRequestAccessible ? [`采购审批 · ${pendingPurchases} 笔 · ${formatMoney(pendingPurchaseCents, true)}`] : []),
    ...(data.summary.paymentApprovalAccessible ? [`付款审批 · ${data.summary.pendingPayments ?? 0} 笔 · ${formatMoney(data.summary.pendingPaymentCents ?? 0, true)}`] : []),
    `金额合计 · ${formatMoney(approvalCents, true)}`,
  ];
  const riskProjectInsight = analytics.projectHealth.slice(0, 3).map((item) => `${item.name} · ${item.score} 分 · ${item.reasons[0] ?? "暂无突出风险"}`);
  const missingInvoiceInsight = analytics.executiveDetails.missingInvoices ? [
    `欠票供应商 · ${analytics.executiveDetails.missingInvoices.supplierCount} 家`,
    `最大欠票供应商 · ${analytics.executiveDetails.missingInvoices.maxSupplierName ?? "暂无"}`,
    `最大欠票金额 · ${formatMoney(analytics.executiveDetails.missingInvoices.maxAmountCents, true)}`,
  ] : [];
  ...
  return <ExecutiveAmbientBackground><main className="content operating-page executive-dashboard os-page-enter">
    <DashboardHero name={user.name} greeting={greetingForShanghai()} date={date} scopeLabel={data.scopeLabel} balanceCents={data.summary.balance} balanceAccessible={data.summary.balanceAccessible} projectedBalanceCents={projectedBalance} gapDate={firstGap ? formatDate(firstGap.date) : undefined} priorities={priorities} aiEnabled={aiBriefEnabled} aiReady={aiReady} />

    <section className="executive-metrics" aria-label="核心经营指标">
      <ExecutiveMetricCard primary label="当前资金余额" valueCents={data.summary.balance} accessible={data.summary.balanceAccessible} detail="在用账户实时汇总" detailTitle="资金构成" insight={currentBalanceInsight} href={financeHref} icon="balance" tone="success" trend={cashTrend} />
      <ExecutiveMetricCard primary label="未来 30 天预计余额" valueCents={projectedBalance} accessible={data.summary.balanceAccessible} detail={firstGap ? `${formatDate(firstGap.date)} 预计出现缺口` : "预测期内暂未发现缺口"} detailTitle="预测构成" insight={[`预计收款 · ${receivable30 === null ? "无权限" : formatMoney(receivable30, true)}`, `预计付款 · ${payable30 === null ? "无权限" : formatMoney(payable30, true)}`, ...(minimumBalance === null ? [] : [`最低余额 · ${formatMoney(minimumBalance, true)}`])]} href={financeHref} icon="balance" tone={firstGap ? "danger" : "success"} trend={cashTrend} />
      <ExecutiveMetricCard label="逾期应收" valueCents={data.summary.overdueReceivables} accessible={data.summary.receivableAccessible} detail={`${analytics.executiveDetails.overdue?.customerCount ?? 0} 个逾期客户`} detailTitle="逾期细分" insight={overdueInsight} href={data.summary.receivableAccessible ? "/receivables?overdue=true" : undefined} icon="receivable" tone={Number(data.summary.overdueReceivables ?? 0) > 0 ? "danger" : "neutral"} />
      <ExecutiveMetricCard label="待审批采购 / 付款" count={approvalCount} accessible={data.summary.purchaseRequestAccessible || data.summary.paymentApprovalAccessible} detail={`合计 ${formatMoney(approvalCents, true)}`} detailTitle="审批细分" insight={approvalInsight} href={approvalHref} icon="approvals" tone={approvalCount ? "warning" : "neutral"} />
      <ExecutiveMetricCard label="风险项目" count={projectRiskCount ?? undefined} accessible={analytics.projectHealthAccessible} detail={`进行中 ${data.summary.activeProjects} 个`} detailTitle="风险项目 Top 3" insight={riskProjectInsight} href={analytics.projectHealthAccessible ? "/projects" : undefined} icon="risk" tone={projectRiskCount ? "danger" : "neutral"} />
      <ExecutiveMetricCard label="供应商欠票" valueCents={data.summary.missingInvoice} accessible={data.summary.payableAccessible} detail={`${analytics.executiveDetails.missingInvoices?.supplierCount ?? 0} 家欠票供应商`} detailTitle="欠票细分" insight={missingInvoiceInsight} href={invoiceHref} icon="invoices" tone={Number(data.summary.missingInvoice ?? 0) > 0 ? "warning" : "neutral"} />
      <ExecutiveMetricCard label="进行中项目" count={data.summary.activeProjects} detail="当前授权项目范围" detailTitle="项目概览" insight={analytics.projectHealth.slice(0, 3).map((item) => `${item.name} · ${item.label}`)} href="/projects" icon="projects" />
    </section>
    ...
    <div className="dashboard-command-grid executive-command-grid">
      <ExecutiveSection kicker="Cashflow Intelligence" title={projectedBalance === null ? "未来 30 天现金流" : `预计期末余额 ${formatMoney(projectedBalance, true)}`} description="当前余额叠加每日预计收款与付款；点击日期进入资金明细。" className="cashflow-command" action={<Landmark aria-hidden="true" />}>
        {analytics.cashflow.length ? <><div className="cashflow-summary"><div><span>当前资金</span><strong>{formatMoney(data.summary.balance ?? 0, true)}</strong></div><div><span>预计收款</span><strong className="success-text">{formatMoney(receivable30 ?? 0, true)}</strong></div><div><span>预计付款</span><strong className="warning-text">{formatMoney(payable30 ?? 0, true)}</strong></div><div><span>最低余额</span><strong className={Number(minimumBalance) < 0 ? "danger-text" : ""}>{formatMoney(minimumBalance ?? 0, true)}</strong></div></div><CashflowAreaChart data={analytics.cashflow} /></> : <ChartEmpty title="当前账号无权查看账户余额" detail="现金流预测未使用 0 代替不可访问的余额。" />}
      </ExecutiveSection>
    ...
    <div className="dashboard-insight-grid">
      <ExecutiveSection kicker="Project Health" title="风险项目优先" description={user.role === "owner" || user.role === "finance" || user.role === "project_manager" ? "健康分综合回款、预算、欠票与现金净额；点击项目进入总账。" : "健康分基于当前可见的预算与交付进度；点击项目进入总账。"} className="project-health-surface" action={<Link className="panel-link" href="/projects">全部项目 <ArrowRight /></Link>}><ProjectHealthChart data={analytics.projectHealth} /></ExecutiveSection>
      <ExecutiveSection kicker="Fund Structure" title="集团资金结构" description={user.role === "owner" ? "悬停查看占比，点击公司切换当前数据范围。" : "悬停查看各公司资金金额与占比。"} className="fund-structure-surface"><CompanyFundsDonut interactive={user.role === "owner"} data={data.balances.map((row) => ({ id: row.id, label: row.name, valueCents: row.balanceCents }))} /><div className="compact-balance-list">{data.balances.map((row) => <div key={row.id}><span>{row.name}<small>{row.accountCount} 个账户</small></span><strong>{formatMoney(row.balanceCents, true)}</strong></div>)}</div></ExecutiveSection>
    </div>

    <div className="executive-aging-grid">
      <ExecutiveSection kicker="Receivable Aging" title="应收账龄" description="点击账龄区间进入应收台账。" className="structure-surface"><ReceivableAgingChart data={analytics.receivableAging} /></ExecutiveSection>
      <ExecutiveSection kicker="Payable Maturity" title="应付到期结构" description="点击到期区间进入应付台账。" className="structure-surface"><PayableMaturityChart data={analytics.payableMaturity} /></ExecutiveSection>
    </div>
  ...
  </main></ExecutiveAmbientBackground>;
}
