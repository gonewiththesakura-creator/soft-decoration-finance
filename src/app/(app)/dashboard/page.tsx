...
export default async function DashboardPage() {
  ...
  const metrics = [
    ...
    { label: "未来 30 天应收", value: formatMoney(data.summary.receivable30, true), meta: `逾期 ${formatMoney(data.summary.overdueReceivables, true)}`, icon: ArrowDownLeft, tone: data.summary.overdueReceivables > 0 ? "warning" : "" },
    { label: "未来 30 天应付", value: formatMoney(data.summary.payable30, true), meta: "按应付节点汇总", icon: ArrowUpRight },
    ...
    { label: "进行中项目", value: `${data.summary.activeProjects} 个`, meta: `${data.overBudget.length} 个项目超预算`, icon: FolderKanban, tone: data.overBudget.length ? "warning" : "" },
  ];
  return <main className="content">
    <div className="page-heading"><div><div className="eyebrow">经营驾驶舱</div><h1>{user.name}，这是今天的经营重点</h1><p className="page-description">资金、未来现金流、审批和项目风险均来自实时业务数据。</p></div><div className="heading-actions"><Link className="button" href="/ai">询问 AI 助手</Link><Link className="button primary" href="/purchase-requests">处理待审批</Link></div></div>
