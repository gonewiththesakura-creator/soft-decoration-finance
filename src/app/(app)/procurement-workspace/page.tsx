...
export default async function ProcurementWorkspacePage({ searchParams }: { searchParams: Promise<{ projectId?: string }> }) { const [user, query] = await Promise.all([requireSession(), searchParams]); if (!can(user, "skus")) notFound(); const scope = await getCompanyScope(user); const data = await getProcurementWorkspace(user, scope); if (data.isEmpty) return <main className="content workspace-page"><EmptyBusinessState canImport={can(user, "imports", "write")} canScan={user.role === "owner"} title="尚未建立真实采购台账" description="先导入公司、项目、供应商和 SKU 明细，再进行询价、比价与采购申请。" /></main>; return <main className="content workspace-page"><div className="page-heading"><div><div className="eyebrow">采购执行</div><h1>采购工作台</h1><p className="page-description">以 SKU 为中心完成询价、比价、选供和采购申请。</p></div></div><ProcurementWorkspace rows={data.rows as Row[]} quotes={data.quotes as Quote[]} canWrite={can(user, "purchase-requests", "write")} initialProjectId={query.projectId ? Number(query.projectId) : undefined} /></main>; }
