...
export default async function ProjectDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return <main className="content operating-page project-os os-page-enter">
    <header className="project-command-header">
      <div className="project-identity"><div className="project-code">{project.code}</div><h1 className="project-name">{project.name}</h1><div className="project-meta"><span><Building2 />{project.companyName}</span>{project.customerId ? <Link href={`/customers/${project.customerId}`}><UserRound />{project.customerName}</Link> : <span><UserRound />{project.customerName}</span>}<span><UserRound />项目经理 {project.managerName}</span><span><UserRound />设计师 {project.designerName}</span><span><CalendarDays />{formatDate(project.startDate)} 至 {formatDate(project.expectedEndDate)}</span><span><MapPin />{project.address || "未填写地址"}</span></div></div>
