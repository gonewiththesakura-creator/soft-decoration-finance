...
export default async function LoginPage() {
  ...
  const companyCount = Number(baseline.companyCount);
  return (
    <main className="login-page">
      <section className="login-context">
        ...
        <div className="login-facts"><div className="login-fact"><strong>{companyCount || "待导入"}</strong><span>{companyCount ? "独立核算公司" : "真实公司数据"}</span></div><div className="login-fact"><strong>{getDataMode() === "real" ? "REAL" : "DEMO"}</strong><span>当前数据模式</span></div><div className="login-fact"><strong>可追溯</strong><span>导入证据链</span></div></div>
      </section>
