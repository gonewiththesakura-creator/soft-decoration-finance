...
export async function GET() {
  ...
  const checks = await sqlQuery<Record<string, unknown>>(`SELECT status,provider,primary_model AS "primaryModel",fast_model AS "fastModel",api_mode AS "apiMode",latency_ms AS "latencyMs",error_code AS "errorCode",created_at::text AS "createdAt" FROM ai_provider_checks ORDER BY created_at DESC LIMIT 10`);
  return Response.json({ configured: config.configured, provider: config.provider, baseUrl: config.baseUrl, primaryModel: config.primaryModel, fastModel: config.fastModel, requestedMode: config.apiMode, store: config.store, timeoutMs: config.timeoutMs, checks });
}
