...
function BucketChart({ data, ariaLabel, hrefBase, colors = agingColors }: { data: BucketPoint[]; ariaLabel: string; hrefBase: string; colors?: string[] }) {
  ...
  return <div className="chart-frame" role="group" aria-label={ariaLabel}><div className="chart-stage compact" onMouseLeave={() => setActiveBucket(null)}><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 8, right: 6, left: 0, bottom: 0 }} onClick={(state) => { const index = Number(state?.activeIndex); const item = Number.isInteger(index) ? data[index] : undefined; if (item) router.push(`${hrefBase}?bucket=${encodeURIComponent(item.label)}`); }} style={{ cursor: "pointer" }}>
    ...
    <Bar name="未结清金额" dataKey="valueCents" radius={[3, 3, 0, 0]} isAnimationActive={!reducedMotion} animationDuration={720}>{data.map((item, index) => <Cell className={`chart-clickable chart-interactive-bar ${activeBucket === item.label ? "is-active" : activeBucket ? "is-muted" : ""}`} key={item.label} fill={colors[index % colors.length]} onMouseEnter={() => setActiveBucket(item.label)} />)}</Bar>
  </BarChart></ResponsiveContainer></div><nav className="chart-drill-links" aria-label={`${ariaLabel}明细入口`}>{data.map((item) => <Link className={activeBucket === item.label ? "is-active" : ""} href={`${hrefBase}?bucket=${encodeURIComponent(item.label)}`} key={item.label} onMouseEnter={() => setActiveBucket(item.label)} onMouseLeave={() => setActiveBucket(null)} onFocus={() => setActiveBucket(item.label)} onBlur={() => setActiveBucket(null)}>{item.label}</Link>)}</nav></div>;
}
