...
export function ProjectHealthChart({ data }: { data: ProjectHealth[] }) {
  ...
  return <div className="chart-frame" role="group" aria-label="项目健康分排行榜，点击进入项目">
    <div className="chart-stage tall" onMouseLeave={() => setActiveProject(null)}><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical" margin={{ top: 6, right: 34, left: 4, bottom: 0 }} onClick={(state) => { const index = Number(state?.activeIndex); const project = Number.isInteger(index) ? data[index] : undefined; if (project?.id) router.push(`/projects/${project.id}`); }} style={{ cursor: "pointer" }}>
      ...
      <Bar name="健康分" dataKey="score" radius={[0, 4, 4, 0]} isAnimationActive={!reducedMotion} animationDuration={780}>{data.map((item) => <Cell className={`chart-clickable chart-interactive-bar ${activeProject === item.id ? "is-active" : activeProject ? "is-muted" : ""}`} key={item.id} fill={toneColor[item.tone]} onMouseEnter={() => setActiveProject(item.id)} />)}<LabelList dataKey="score" position="right" className="health-score-label" /></Bar>
    ...
    <nav className="chart-drill-links project-drill-links" aria-label="项目健康明细入口">{data.map((project) => <Link className={activeProject === project.id ? "is-active" : ""} key={project.id} href={`/projects/${project.id}`} onMouseEnter={() => setActiveProject(project.id)} onMouseLeave={() => setActiveProject(null)} onFocus={() => setActiveProject(project.id)} onBlur={() => setActiveProject(null)}>{project.name} · {project.score}</Link>)}</nav>
  </div>;
}
