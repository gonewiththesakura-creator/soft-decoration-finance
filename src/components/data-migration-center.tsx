...
type SheetInfo = {
  id: number; name: string; rowCount: number; columnCount: number; headers: string[]; previewRows: Record<string, unknown>[];
...
const migrationOrder = ["数据归属", "项目确认", "自动分析", "异常确认", "暂存预检", "人工确认", "正式写入", "血缘归档"];
...
export function DataMigrationCenter({ allowedTypes, dataMode, initialMode, isOwner }: { allowedTypes: string[]; dataMode: "real" | "demo"; initialMode: "upload" | "folder"; isOwner: boolean }) {
  ...
  async function confirmContext(overrideConflict = false) {
    ...
    try {
      ...
      setMessage(result.conflict ? "发现项目冲突，已暂停自动导入" : `数据归属已确认：${scopeLabels[scope]}${result.project ? ` · ${result.project.name}` : ""}`);
  ...
  async function stage() {
    if (!workbook || !selectedSheet) return; setLoading(true); setMessage("正在按已确认项目执行关联匹配、重复检测与业务校验...");
    try {
  ...
  return <>
    ...
    <div className="migration-stack">
      ...
      {workbook?.contextConfirmed ? <AnalysisPanel workbook={workbook} facts={facts} pendingFacts={pendingFacts} projectLabel={projectLabel} selectedSheet={selectedSheet} loading={loading} reviewFact={reviewFact} stage={stage} /> : null}
      ...
      {selectedSheet?.headers.length ? <section className="panel advanced-mapping-panel"><button className="advanced-mapping-toggle" onClick={() => setAdvancedOpen((value) => !value)} aria-expanded={advancedOpen}><span><strong>高级字段映射</strong><small>自动识别不足时再手工选择业务类型与字段</small></span><ChevronDown className={advancedOpen ? "is-open" : ""} /></button>{advancedOpen ? <><div className="mapping-toolbar"><div className="field"><label>业务类型</label><select className="input" value={businessType} onChange={(event) => setBusinessType(event.target.value)}>{migrationDefinitions.filter((item) => allowedTypes.includes(item.resource)).map((item) => <option key={item.resource} value={item.resource}>{item.label}</option>)}</select></div><div className="field"><label>保存 Mapping Template（可选）</label><input className="input" value={templateName} onChange={(event) => setTemplateName(event.target.value)} placeholder="例如：项目采购总表格式" /></div></div><div className="table-scroll"><table className="data-table mapping-table"><thead><tr><th>Excel 字段</th><th>系统字段</th><th>匹配结果</th></tr></thead><tbody>{selectedSheet.headers.map((header) => <tr key={header}><td className="table-primary">{header}</td><td><select className="input" value={mappings[header] ?? ""} onChange={(event) => setMappings((current) => ({ ...current, [header]: event.target.value }))}><option value="">保留原始证据 / 不结构化</option>{fields.map((field) => <option key={field.key} value={field.key}>{field.displayLabel}{field.required ? " *" : ""}</option>)}</select></td><td><span className={`badge ${mappings[header] ? "success" : ""}`}>{mappings[header] ? "已映射" : "不结构化"}</span></td></tr>)}</tbody></table></div><div className="modal-footer"><button className="button primary" onClick={() => void stage()} disabled={loading || !workbook?.contextConfirmed}>{loading ? <LoaderCircle className="animate-spin" /> : <CheckCircle2 />}按手动映射预检</button></div></> : null}</section> : null}
      ...
      {detail?.rows.length ? <StagingPanel detail={detail} fields={fields} batchStatus={batchStatus} errorRows={errorRows} pendingFacts={pendingFacts} loading={loading} resolutionsByRow={resolutionsByRow} loadBatch={loadBatch} updateRow={updateRow} setEditRow={setEditRow} batchAction={batchAction} /> : null}
...
function ContextPanel(props: {
  ...
  const disabled = props.loading || (props.scope === "PROJECT" && props.projectMode === "new" && !props.projectName.trim()) || (props.scope === "PROJECT" && props.projectMode === "existing" && !props.selectedProjectId) || (props.scope === "COMPANY" && props.companyMode === "existing" && !props.selectedCompanyId) || (props.scope === "COMPANY" && props.companyMode === "new" && !props.companyName.trim());
  return <section className="panel import-context-panel"><div className="panel-header"><div><div className="panel-title">2. 确认数据归属</div><div className="panel-subtitle">项目数据必须在暂存前绑定项目；公司级和主数据使用各自作用域</div></div>{props.workbook.contextConfirmed ? <span className="badge success"><CheckCircle2 />已锁定</span> : <span className="badge warning">待确认</span>}</div><div className="scope-selector" role="radiogroup" aria-label="数据归属"><button className={props.scope === "PROJECT" ? "active" : ""} onClick={() => props.setScope("PROJECT")}><BriefcaseBusiness /><span><strong>项目数据</strong><small>成本、采购、付款、发票等</small></span></button><button className={props.scope === "COMPANY" ? "active" : ""} onClick={() => props.setScope("COMPANY")}><Building2 /><span><strong>公司级数据</strong><small>账户、公司汇总等</small></span></button><button className={props.scope === "MASTER" ? "active" : ""} onClick={() => props.setScope("MASTER")}><Library /><span><strong>公共主数据</strong><small>供应商、客户、字典等</small></span></button></div>{props.scope === "PROJECT" ? <div className="context-form"><div className="context-mode"><label><input type="radio" checked={props.projectMode === "new"} onChange={() => props.setProjectMode("new")} />新建项目</label><label><input type="radio" checked={props.projectMode === "existing"} onChange={() => props.setProjectMode("existing")} />导入到已有项目</label></div>{props.projectMode === "existing" ? <div className="field"><label>目标项目</label><select className="input" value={props.selectedProjectId} onChange={(event) => props.setSelectedProjectId(event.target.value)}><option value="">请选择项目</option>{props.overview?.projects.map((project) => <option key={project.id} value={project.id}>{project.code} · {project.name} · {project.companyName}</option>)}</select></div> : <div className="context-grid"><div className="field"><label>项目名称 *</label><input className="input" value={props.projectName} onChange={(event) => props.setProjectName(event.target.value)} placeholder="例如：青岛鑫江中心" /></div><div className="field"><label>项目编号（可选）</label><input className="input" value={props.projectCode} onChange={(event) => props.setProjectCode(event.target.value)} placeholder="留空自动生成" /></div><div className="field"><label>所属公司（可后补）</label><select className="input" value={props.selectedCompanyId} onChange={(event) => props.setSelectedCompanyId(event.target.value)}><option value="">暂不绑定公司</option>{props.overview?.companies.map((company) => <option key={company.id} value={company.id}>{company.code} · {company.name}</option>)}</select></div></div>}</div> : props.scope === "COMPANY" ? <div className="context-form"><div className="context-mode"><label><input type="radio" checked={props.companyMode === "existing"} onChange={() => props.setCompanyMode("existing")} />选择公司</label>{props.isOwner ? <label><input type="radio" checked={props.companyMode === "new"} onChange={() => props.setCompanyMode("new")} />新建公司</label> : null}</div>{props.companyMode === "existing" ? <div className="field"><label>所属公司</label><select className="input" value={props.selectedCompanyId} onChange={(event) => props.setSelectedCompanyId(event.target.value)}><option value="">请选择公司</option>{props.overview?.companies.map((company) => <option key={company.id} value={company.id}>{company.code} · {company.name}</option>)}</select></div> : <div className="field"><label>公司名称 *</label><input className="input" value={props.companyName} onChange={(event) => props.setCompanyName(event.target.value)} /></div>}</div> : <div className="context-master-note"><Library /><span>公共主数据不绑定项目，供应商和客户仍保持共享主记录。</span></div>}<div className="migration-footer"><span>{props.workbook.analysis.projectCandidate ? `系统候选项目：${props.workbook.analysis.projectCandidate.name} · ${Math.round(props.workbook.analysis.projectCandidate.confidence / 100)}%` : "未从文件中提取到明确项目名称"}</span><button className="button primary" disabled={disabled} onClick={() => void props.confirmContext()}><CheckCircle2 />确认数据归属</button></div></section>;
}

function AnalysisPanel(props: { workbook: Workbook; facts: StoredFact[]; pendingFacts: StoredFact[]; projectLabel: string; selectedSheet: SheetInfo | null; loading: boolean; reviewFact: (id: number, decision: "ACCEPT" | "IGNORE") => Promise<void>; stage: () => Promise<void> }) {
  const displayFacts: Array<StoredFact | RecognizedBusinessFact> = props.facts.length ? props.facts : props.workbook.analysis.facts;
  return <section className="panel import-analysis-panel"><div className="panel-header"><div><div className="panel-title">3. 系统自动分析</div><div className="panel-subtitle">综合文件名、Sheet、表头、样本类型和结构信号</div></div><span className={`badge ${props.workbook.analysis.overallConfidence >= 9000 ? "success" : "warning"}`}><Sparkles />综合置信度 {Math.round(props.workbook.analysis.overallConfidence / 100)}%</span></div><div className="analysis-summary"><div><span>目标作用域</span><strong>{scopeLabels[props.workbook.scope]}</strong></div><div><span>目标项目</span><strong>{props.projectLabel}</strong></div><div><span>高置信业务事实</span><strong>{props.workbook.analysis.highConfidenceCount}</strong></div><div><span>需要人工确认</span><strong>{props.pendingFacts.length}</strong></div></div><div className="fact-plan-list">{displayFacts.map((fact, index) => { const stored = "id" in fact ? fact : null; const label = stored?.payload.label ?? (fact as RecognizedBusinessFact).label ?? fact.factType; const count = stored?.payload.plannedCount ?? (fact as RecognizedBusinessFact).plannedCount ?? 1; const sheetName = stored?.payload.sheetName ?? (fact as RecognizedBusinessFact).sheetName; const level = (stored?.confidenceLevel ?? (fact as RecognizedBusinessFact).level) as ConfidenceLevel; const status = stored?.status ?? (level === "HIGH" ? "AUTO_ACCEPTED" : "REQUIRES_CONFIRMATION"); return <div className="fact-plan-row" key={stored?.id ?? `${fact.factType}-${index}`}><div><strong>{label}</strong><span>{sheetName} · 计划 {count} 项</span></div><span className={`badge ${level === "HIGH" ? "success" : level === "LOW" || level === "CONFLICT" ? "danger" : "warning"}`}>{status === "USER_ACCEPTED" ? "已确认" : status === "IGNORED" ? "保留证据 / 忽略" : confidenceLabels[level]}</span>{stored && status === "REQUIRES_CONFIRMATION" ? <div className="fact-actions"><button className="button small" disabled={props.loading} onClick={() => void props.reviewFact(stored.id, "IGNORE")}>忽略结构化</button><button className="button small" disabled={props.loading} onClick={() => void props.reviewFact(stored.id, "ACCEPT")}>确认识别</button></div> : null}</div>; })}</div><div className="migration-footer"><span>{props.pendingFacts.length ? `还有 ${props.pendingFacts.length} 项需要处理` : "高置信项已自动接受，可进入暂存预检"}</span><button className="button primary" disabled={props.loading || !props.selectedSheet || props.pendingFacts.length > 0} onClick={() => void props.stage()}><CheckCircle2 />进入暂存预检</button></div></section>;
}
