...
import { Ban, Check, Download, Eye, FileQuestion, LoaderCircle, Pencil, Plus, Search, X } from "lucide-react";
...
export function ResourceTable({ definition, rows, canWrite, role }: { definition: ResourceDefinition; rows: Row[]; canWrite: boolean; role: string }) {
  const router = useRouter(); const [query, setQuery] = useState(""); const [showForm, setShowForm] = useState(false); const [editingRow, setEditingRow] = useState<Row | null>(null); const [approval, setApproval] = useState<ApprovalTarget | null>(null); const [error, setError] = useState(""); const [toast, setToast] = useState(""); const [pending, startTransition] = useTransition();
  ...
  const hasActions = (canWrite && (editableResources.has(definition.key) || voidableResources.has(definition.key))) || (approvalResource && (role === "owner" || (definition.key === "payment-requests" && role === "finance")));
  return <>
    ...
    {approval ? <ApprovalDetail {...approval} role={role} onClose={() => setApproval(null)} onDone={approvalDone} /> : null}{toast ? <div className="toast">{toast}</div> : null}
  </>;
}
