...
export const projectTabs = [
  ...
  ["invoices", "发票"], ["budgets", "预算 / 变更"], ["returns", "退换货"], ["source-files", "原始文件"], ["imports", "导入记录"], ["attachments", "附件"], ["logs", "操作日志"],
] as const;
...
export async function getProjectDetail(id: number, user: SessionUser) {
  ...
  if (user.role !== "owner" && !["project_manager", "designer"].includes(user.role)) { params.push(user.companyId); checks.push(`p.company_id=$${params.length}`); }
  if (["project_manager", "designer"].includes(user.role)) { params.push(user.id); checks.push(`EXISTS (SELECT 1 FROM project_members pm WHERE pm.project_id=p.id AND pm.user_id=$${params.length})`); params.push(user.companyId); checks.push(`(p.company_id IS NULL OR p.company_id=$${params.length})`); }
  const [project] = await sqlQuery<{
    id: number; companyId: number | null; customerId: number | null; code: string; name: string; companyName: string; customerName: string; managerName: string; designerName: string;
  ...
  }>(`SELECT p.id,p.company_id AS "companyId",p.customer_id AS "customerId",p.code,p.name,COALESCE(c.name,'待补充公司') AS "companyName",COALESCE(cu.name,'待补充客户') AS "customerName",COALESCE(pm.name,'-') AS "managerName",COALESCE(d.name,'-') AS "designerName",p.address,p.status,p.current_contract_cents AS "currentContractCents",p.original_contract_cents AS "originalContractCents",p.budget_cents AS "budgetCents",p.start_date::text AS "startDate",p.expected_end_date::text AS "expectedEndDate",p.warranty_end_date::text AS "warrantyEndDate",p.audit_status AS "auditStatus" FROM projects p LEFT JOIN companies c ON c.id=p.company_id LEFT JOIN customers cu ON cu.id=p.customer_id LEFT JOIN users pm ON pm.id=p.manager_id LEFT JOIN users d ON d.id=p.designer_id WHERE ${checks.join(" AND ")}`, params);
...
const tabQueries: Record<Exclude<ProjectTab, "overview" | "attachments">, ReturnType<typeof table>[]> = {
  ...
  "source-files": [table("项目原始文件", [{ key: "filename", label: "文件名" }, { key: "versionNumber", label: "版本", format: "number" }, { key: "source", label: "来源" }, { key: "hash", label: "SHA-256" }, { key: "fingerprintStatus", label: "状态", format: "status" }, { key: "createdAt", label: "上传时间", format: "date" }], `SELECT f.id,f.filename,f.version_number AS "versionNumber",f.channel AS source,f.file_hash AS hash,f.fingerprint_status AS "fingerprintStatus",f.created_at::text AS "createdAt" FROM import_files f WHERE f.project_id=$1 ORDER BY f.created_at DESC`)],
  imports: [table("项目导入记录", [{ key: "batchNumber", label: "批次" }, { key: "filename", label: "文件" }, { key: "scopeType", label: "数据归属", format: "status" }, { key: "sourceChannel", label: "来源" }, { key: "businessType", label: "业务类型" }, { key: "successRows", label: "写入", format: "number" }, { key: "skippedRows", label: "跳过", format: "number" }, { key: "status", label: "状态", format: "status" }, { key: "createdAt", label: "创建时间", format: "date" }], `SELECT b.id,b.batch_number AS "batchNumber",f.filename,b.scope_type AS "scopeType",b.source_channel AS "sourceChannel",COALESCE(b.business_type,'自动识别') AS "businessType",b.success_rows AS "successRows",b.skipped_rows AS "skippedRows",b.status,b.created_at::text AS "createdAt" FROM import_batches b LEFT JOIN import_files f ON f.batch_id=b.id WHERE b.project_id=$1 ORDER BY b.created_at DESC`)],
