...
export const definitions: Partial<Record<ResourceKey, ResourceDefinition>> = {
  projects: {
    ...
    query: `SELECT p.id,p.company_id AS "companyId",p.customer_id AS "customerId",p.code,p.name,p.address,COALESCE(c.name,'待补充公司') AS "companyName",COALESCE(cu.name,'待补充客户') AS "customerName",u.name AS "managerName",p.original_contract_cents AS "originalContractCents",p.current_contract_cents AS "contractCents",p.budget_cents AS "budgetCents",p.start_date::text AS "startDate",p.expected_end_date::text AS "expectedEndDate",COALESCE((SELECT sum(po.amount_cents) FROM purchase_orders po WHERE po.project_id=p.id AND NOT po.is_void),0)::float8 AS "orderedCents",COALESCE((SELECT sum(r.amount_cents) FROM receipts r WHERE r.project_id=p.id AND NOT r.is_void),0)::float8 AS "receivedCents",CASE WHEN COALESCE((SELECT sum(po.amount_cents) FROM purchase_orders po WHERE po.project_id=p.id AND NOT po.is_void),0)>p.budget_cents THEN '超预算' WHEN COALESCE((SELECT sum(rp.amount_cents-rp.received_cents) FROM receivable_plans rp WHERE rp.project_id=p.id AND rp.due_date<now() AND NOT rp.is_void AND NOT rp.is_warranty),0)>0 THEN '回款逾期' ELSE '正常' END AS risk,p.status FROM projects p LEFT JOIN companies c ON c.id=p.company_id LEFT JOIN customers cu ON cu.id=p.customer_id LEFT JOIN users u ON u.id=p.manager_id WHERE {scope} ORDER BY p.updated_at DESC`, alias: "p", projectExpression: "p.id", emptyTitle: "暂无项目",
  },
  companies: {
  ...
  "purchase-requests": {
    ...
    fields: [{ key: "projectId", label: "所属项目", type: "reference", reference: "projects", required: true }, { key: "supplierId", label: "供应商", type: "reference", reference: "suppliers", required: true }, { key: "skuId", label: "采购 SKU", type: "reference", reference: "skus", required: true }, { key: "number", label: "申请编号", type: "text", required: true }, { key: "paymentType", label: "付款类型", type: "select", required: true, options: [{ value: "对公", label: "对公" }, { value: "批量私账", label: "批量私账" }] }, { key: "amountYuan", label: "申请金额（元）", type: "number", required: true }, { key: "reason", label: "采购说明", type: "textarea", required: true }],
