...
CREATE TABLE IF NOT EXISTS skus (id serial PRIMARY KEY, company_id integer NOT NULL REFERENCES companies(id), project_id integer NOT NULL REFERENCES projects(id), code text UNIQUE NOT NULL, room text, category text NOT NULL, name text NOT NULL, brand text, model text, specification text, material text, color text, quantity integer NOT NULL, unit text NOT NULL, budget_unit_cents integer NOT NULL, final_unit_cents integer, supplier_id integer REFERENCES suppliers(id), tax_rate_bps integer NOT NULL DEFAULT 1300, freight_cents integer NOT NULL DEFAULT 0, install_cents integer NOT NULL DEFAULT 0, lead_days integer, status text NOT NULL DEFAULT '待询价', note text, created_at timestamptz NOT NULL DEFAULT now(), created_by integer, updated_at timestamptz NOT NULL DEFAULT now(), updated_by integer);
...
CREATE TABLE IF NOT EXISTS invoices (id serial PRIMARY KEY, company_id integer NOT NULL REFERENCES companies(id), project_id integer NOT NULL REFERENCES projects(id), customer_id integer REFERENCES customers(id), supplier_id integer REFERENCES suppliers(id), number text UNIQUE NOT NULL, direction text NOT NULL, type text NOT NULL, amount_cents integer NOT NULL, issued_at timestamptz NOT NULL, status text NOT NULL DEFAULT '正常', version integer NOT NULL DEFAULT 1, is_void boolean NOT NULL DEFAULT false, void_reason text, created_at timestamptz NOT NULL DEFAULT now(), created_by integer, updated_at timestamptz NOT NULL DEFAULT now(), updated_by integer);
